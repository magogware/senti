...
onready var capsule = get_node("Target/PlayerPos/Capsule")
...
func _physics_process(delta):
	...
	if controller and controller.get_is_active() and (abs(controller.get_joystick_axis(JOY_OPENVR_TOUCHPADX)) > 0.1 || abs(controller.get_joystick_axis(JOY_OPENVR_TOUCHPADY)) > 0.1):# or controller.is_button_pressed(teleport_button):
		...
		if hit_something:
			...
			teleport_rotation = (controller.get_joystick_axis(4)*PI*2)
